	...
	<div class="device"> <!-- Container voor ieder invidiueel device -->
		...
		<h1>Chair</h1>
			...
			<input type="submit" name = "button4" class="button" value="Massage <?php //global $arr; echo $arr[4]; ?>"> <!-- Vibrator, Chair -->
					<?php
					if (isset($_POST['button4'])){global $arr; if ($arr[4] == 0){$arr[4] = 2;}else {$arr[4] = 0;}Write("4,$arr[4]"); refresh(0);}
					?>
		...
		<h1>Column</h1>
			...
			<input type="submit" name = "button11" class="button" value="Led (Column) <?php //global $arr; echo $arr[11]; ?>"> <!-- Led, Column -->
				...
				<div class="led-group">
					<div class="led <?php global $arr; if($arr[13] <= 500){echo "red";}else{echo "grey";}?>" value="Smoke"><p><?php global $arr; if($arr[13] <= 500){echo "ON";}else{echo "OFF";}?></p></div><!-- Smoke, Column -->
