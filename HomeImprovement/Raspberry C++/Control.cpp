...
Control::Control() 
{
    ...
    Device *bed_dev = new Bed("10.42.0.123", s1);           // create new device
    addDevice(1, bed_dev);                                  // add device to map for devices

    Database *bed_dat = new Database();                     // create new database object
    addDatabase(1, bed_dat);                                // add database object to map for databases

    Device *stoel_dev = new Stoel("10.42.0.170", s1);       // create new device
...
void Control::getData()
{
    ...
    {
        int b = dat->second->receiveMessage();          // get data from json file, NOTE: Deze functie moet nog ingevuld worden, er staat nu fake data in
...
void Control::compareDatabaseToDevice()
{
    ...
    for (map<int, Database*>::iterator dat = databases.begin(); dat != databases.end(); ++dat)
    {
    	map<int, Device*>::iterator dev = devices.find(dat->first); // find device with the same key value as the database object

        // compare values 
        if(!(dat->second->getValue() == dev->second->getValue())) 
        {
